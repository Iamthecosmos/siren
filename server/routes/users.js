...
router.get("/stats/overview", requireAdmin, async (req, res, next) => {
  try {
    ...
    const verifiedUsers = await getQuery(
    ...
    const totalReports = await getQuery(
      'SELECT COUNT(*) as count FROM community_reports WHERE status = "active"',
    );
    const verifiedReports = await getQuery(
      "SELECT COUNT(*) as count FROM community_reports WHERE is_verified = 1",
    );
    ...
    // Voice statistics
    const totalVoices = await getQuery(
      "SELECT COUNT(*) as count FROM voice_contributions WHERE is_approved = 1",
    );
    const pendingVoices = await getQuery(
      "SELECT COUNT(*) as count FROM voice_contributions WHERE is_approved = 0",
    );
    ...
    // Activity statistics
    const totalDownloads = await getQuery(
      "SELECT SUM(download_count) as total FROM voice_contributions",
    );
    const totalVerifications = await getQuery(
      "SELECT COUNT(*) as count FROM report_verifications",
    );
    ...
    res.json({
      users: {
        ...
        verificationRate:
          totalUsers.count > 0
            ? ((verifiedUsers.count / totalUsers.count) * 100).toFixed(1)
            : 0,
      },
      reports: {
        ...
        verificationRate:
          totalReports.count > 0
            ? ((verifiedReports.count / totalReports.count) * 100).toFixed(1)
            : 0,
      },
      voices: {
        ...
        approvalRate:
          totalVoices.count + pendingVoices.count > 0
            ? (
                (totalVoices.count /
                  (totalVoices.count + pendingVoices.count)) *
                100
              ).toFixed(1)
            : 0,
      },
