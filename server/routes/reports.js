...
const createReportValidation = [
  ...
  body('incidentType')
    .isIn(['theft', 'assault', 'harassment', 'vandalism', 'suspicious', 'drug', 'weapon', 'accident', 'other'])
...
router.get('/', optionalAuth, [
  ...
  query('type').optional().isIn(['theft', 'assault', 'harassment', 'vandalism', 'suspicious', 'drug', 'weapon', 'accident', 'other']),
  ...
  try {
    ...
    if (req.query.lat && req.query.lng) {
      ...
      const lngDelta = radius / (111 * Math.cos(req.query.lat * Math.PI / 180));
    ...
    const reports = await allQuery(reportsQuery, [...queryParams, limit, offset]);
...
router.post('/', authenticateToken, createReportValidation, async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    res.status(201).json({
      ...
      report: {
        ...
        photos: createdReport.photos_json ? JSON.parse(createdReport.photos_json) : [],
...
router.get('/area/:lat/:lng', optionalAuth, [
  query('radius').optional().isFloat({ min: 0.1, max: 100 }).withMessage('Radius must be between 0.1 and 100 km')
], async (req, res, next) => {
  try {
