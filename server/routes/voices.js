...
router.get('/', optionalAuth, [
  ...
  query('type').optional().isIn(['emergency', 'casual', 'professional', 'family', 'custom']),
  ...
  try {
    ...
    const voices = await allQuery(voicesQuery, [...queryParams, limit, offset]);
...
router.post('/', authenticateToken, upload.single('voiceFile'), createVoiceValidation, async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
...
router.post('/:id/rate', authenticateToken, rateVoiceValidation, async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
